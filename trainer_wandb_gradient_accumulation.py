# trainer_wandb_gradient_accumulation.py
# Training script with gradient accumulation support
# Allows simulating batch_size * accumulation_steps while fitting smaller batches in memory
# Example: batch_size=4 + accumulation_steps=4 = effective batch_size=16
...
# ----------------------------- Main -----------------------------
def main():
    parser = argparse.ArgumentParser(description="Training script with gradient accumulation (W&B only, model-agnostic)")
    ...
    parser.add_argument('--batch_size', type=int, default=4,
                        help='Batch size per forward pass (memory-limited batch)')
    parser.add_argument('--accumulation_steps', type=int, default=4,
                        help='Number of accumulation steps. Effective batch = batch_size * accumulation_steps.')
    ...
    print(f"Gradient Accumulation: batch_size={args.batch_size} × accumulation_steps={args.accumulation_steps} = effective_batch_size={effective_batch_size}")
    ...
    # ---------------- LR schedule: linear warmup + cosine decay (per optimizer update) ----------------
    updates_per_epoch = max(1, math.ceil(len(train_loader) / args.accumulation_steps))
    total_steps = max(1, args.epochs * updates_per_epoch)
    warmup_steps = max(0, int(round(args.warmup_epochs * updates_per_epoch)))
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Make sure to perform final step if there are remaining accumulated gradients
        if accumulation_counter % args.accumulation_steps != 0:
            optimizer.step()
            optimizer.zero_grad(set_to_none=True)
            scheduler.step()
        ...
        print(
            ...
            f"RMSE(HU): {avg_rmse:.2f} \u00b1 {std_rmse:.2f}"
        )
