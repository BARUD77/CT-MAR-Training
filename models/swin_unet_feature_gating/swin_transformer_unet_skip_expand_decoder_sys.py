...
# patched to accept and use a soft artifact map (A_MG) injected into WindowAttention as a per-window bias.
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None, metal_bias=None, artifact_scores=None, artifact_lambda=None):
        ...
        q, k, v = qkv[0], qkv[1], qkv[2]  # q,k,v: (B_, nH, N, head_dim)
        ...
        if metal_bias is not None:
            # expect metal_bias shape (B_, N) - cast to match dtype/device
            mb = metal_bias.to(attn.dtype).to(attn.device)
            mb = (self.metal_scale * mb).view(B_, 1, 1, -1)  # (B_,1,1,N)
            attn = attn + mb  # broadcast across heads and query positions
...
class SwinTransformerBlock(nn.Module):
    r""" Swin Transformer Block with optional artifact_map input """
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        # prepare attention mask for shifted windows
    ...
    def forward(self, x, artifact_map=None, artifact_lambda=None):
        ...
        # cyclic shift with corresponding shift for artifact map
        if self.shift_size > 0:
            shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
            if artifact_map is not None:
                am = artifact_map
                if am.dim() == 4 and am.shape[1] == 1:
                    am = am[:, 0]
                am_shifted = torch.roll(am, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
            else:
                am_shifted = None
        else:
            shifted_x = x
        ...
        # partition windows for features
        ...
        # partition windows for artifact_map -> (B_, N)
        if am_shifted is not None:
            am_windows = window_partition(am_shifted.unsqueeze(-1), self.window_size)  # (B_, w, w, 1)
            am_windows = am_windows.view(-1, self.window_size * self.window_size)  # (B_, N)
            # optional normalization: ensure values in reasonable range (caller should normalize A_MG)
            # am_windows = am_windows.clamp(0.0, 1.0)
        else:
            am_windows = None
        ...
        # reverse cyclic shift on features
        if self.shift_size > 0:
...
class BasicLayer_up(nn.Module):
    """ Basic layer for decoder (upsampling). Accepts artifact_map_stage optionally (ignored by default). """
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0.,
...
class SwinTransformerSys(nn.Module):
    r""" Swin Transformer UNet system with artifact map injection. """
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        # Feature-gating ablation: learnable scalar applied at the bottleneck.
        # gated = bottleneck * (1 + gamma * artifact_resized)
    ...
    def forward_features(self, x, artifact_map=None, artifact_lambda=None):
        ...
        # prepare artifact map base
        if artifact_map is not None:
            am = artifact_map
            if am.dim() == 4 and am.shape[1] == 1:
                am = am[:, 0]  # (B, H_img, W_img)
        else:
            am = None

        x_downsample = []
        for i_layer, layer in enumerate(self.layers):
            # compute stage resolution (tokens)
            Hs = self.patches_resolution[0] // (2 ** i_layer)
            Ws = self.patches_resolution[1] // (2 ** i_layer)
    ...
    def forward_up_features(self, x, x_downsample, artifact_map=None, artifact_lambda=None):
        ...
        artifact_map: currently unused in decoder (could be used if desired)
        """
    ...
    def forward(self, x, artifact_map=None, artifact_lambda=None):
        ...
        # Feature gating at the bottleneck only.
        # x has shape (B, L, C) where L = Hb * Wb at the deepest stage.
        if artifact_map is not None:
            B, L, C = x.shape
            Hb = self.patches_resolution[0] // (2 ** (self.num_layers - 1))
            Wb = self.patches_resolution[1] // (2 ** (self.num_layers - 1))
            assert L == Hb * Wb, f"bottleneck token count {L} != {Hb}*{Wb}"

            am = artifact_map
            if am.dim() == 3:
                am = am.unsqueeze(1)  # (B,1,H,W)
            # Resize artifact map to bottleneck spatial resolution.
            am_b = F.interpolate(am, size=(Hb, Wb), mode='bilinear', align_corners=False)  # (B,1,Hb,Wb)

            # Reshape features to (B, C, Hb, Wb), gate, and reshape back.
            x_2d = x.transpose(1, 2).reshape(B, C, Hb, Wb)
            x_2d = x_2d * (1.0 + self.gamma * am_b)  # broadcasts over C
            x = x_2d.reshape(B, C, L).transpose(1, 2).contiguous()
