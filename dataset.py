...
class CTMetalArtifactDataset(Dataset):
    ...
    def __init__(
        ...
        ma_pat   = re.compile(r'^Input_Image_(\d+)_\d+x\d+(?:\.npy)?$',      re.IGNORECASE)
        gt_pat   = re.compile(r'^Simulated_Image_(\d+)_\d+x\d+(?:\.npy)?$',  re.IGNORECASE)
        ...
        # Apply head/body policy via numeric cutoff
        if head_policy == "exclude":  # body-only
            common_ids = [i for i in common_ids if i <= id_max_body]
        elif head_policy == "only":   # head-only
            common_ids = [i for i in common_ids if i > id_max_body]
        # else 'all': keep all
        ...
        print(
            ...
            f"| ids=[{min_id}..{max_id}] | head_policy={head_policy} | id_max_body={id_max_body} "
